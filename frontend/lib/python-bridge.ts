...
const dbQueryCliPath = path.join(workspaceRoot, "backend", "app", "cli", "db_query_cli.py");
...
export function runPythonDbQuery(command: "tables" | "query", payload?: unknown): Promise<unknown> {
  return new Promise((resolve, reject) => {
    const args = [dbQueryCliPath, command];
    const child = spawn(pythonCommand, args, {
      cwd: workspaceRoot,
      env: pythonEnv
    });
