...
function NavIcon({ icon, active }: { icon: string; active: boolean }) {
  const activeFillColor = "#012E66";
  const activeStrokeColor = "#FF9026";
  const strokeColor = active ? activeStrokeColor : "black";
  const fillColor = active ? activeFillColor : "none";
  ...
  if (icon === "home") {
    return (
      <svg viewBox="0 0 24 24" className="h-7 w-7" fill="none" stroke={strokeColor} strokeWidth="2.2" strokeLinejoin="round" aria-hidden="true">
        <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" fill={fillColor} />
  ...
  if (icon === "strategy") {
    return (
      <svg viewBox="0 0 24 24" className="h-7 w-7" fill="none" stroke={strokeColor} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        <path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2" fill={fillColor} />
  ...
  if (icon === "execution") {
    return (
      <svg viewBox="0 0 24 24" className="h-7 w-7" fill="none" stroke={strokeColor} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        <polyline points="8 12 11 15 16 9" stroke={strokeColor} />
      </svg>
    );
  }
      ...
      {items.map((item) => {
        ...
        return (
          <button
            ...
            className={`flex w-1/5 flex-col items-center gap-1 text-[11px] font-medium ${
              isActive ? "text-[#012E66]" : "text-black"
            }`}
          >
