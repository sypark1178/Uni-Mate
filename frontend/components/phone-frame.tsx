...
export function PhoneFrame({ children, title, subtitle, fullBleed = false }: PhoneFrameProps) {
  return (
    <main className="app-shell flex min-h-screen justify-center px-4 py-6">
      <section className="relative h-[812px] w-[375px] overflow-hidden bg-white shadow-soft">
        <header className="absolute left-0 right-0 top-0 z-20 flex h-11 items-center justify-between px-[21px]">
          <div className="text-[15px] font-semibold text-black">9:41</div>
          <div className="flex items-center gap-[5px]">
            <svg viewBox="0 0 17 11" className="h-[11px] w-[17px]" fill="none" aria-hidden="true">
            ...
            <svg viewBox="0 0 17 13" className="h-[13px] w-[17px]" fill="none" aria-hidden="true">
            ...
            <svg viewBox="0 0 25 12" className="h-3 w-[25px]" fill="none" aria-hidden="true">
        ...
        {(title || subtitle) && (
          <header className="px-6 pb-3 pt-16">
        ...
        <div className={fullBleed ? "h-full" : "px-6 pb-28 pt-14"}>{children}</div>
      </section>
    </main>
  );
}
